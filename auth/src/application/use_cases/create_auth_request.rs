...
pub async fn execute<T, U, ES>(
    ...
    let auth_request =
        match CreateAuthRequest::new(payload.password, confirmation_code, identity.clone()) {
            Ok(new_user) => new_user,
            Err(error) => return Err(CreateError::InvalidData(error.to_string())),
        };
    // verify no identity in cache
    match repo_cache
        .find_by_id::<CreateAuthRequest>(cache_conn, auth_request.identity.get_value().clone())
        .await
    {
        Ok(user) => {
            if user.is_some() {
                return Err(CreateError::Conflict(
                    "identification id unavailable".to_string(),
                ));
            }
        }
